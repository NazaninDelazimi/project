...
"                       __global float *b,                       \n" \
"                       __global float *c,                       \n" \
"                       const unsigned int n,const unsigned int i,                    \n" \
"                       const unsigned int j,const unsigned int k,                    \n" \
"                       const unsigned int bl_size,const unsigned int im_width)                    \n" \

"{                                                               \n" \
"    a= a+ k*n ; b = b+ k*n; c = c+ k*n;//Get our global thread ID                                  \n" \
...
unsigned int i,j,k,b,bl_size,im_width;
bl_size =100;
im_width = 100;
for(i = 0 ; i < 1 ; i++){
	for (j = 0 ; j < 1 ; j++){
    ...
    err |= clSetKernelArg(kernel, 3, sizeof(unsigned int), &bl_size);
    err |= clSetKernelArg(kernel, 4, sizeof(unsigned int), &i);
    err |= clSetKernelArg(kernel, 5, sizeof(unsigned int), &j);
    err |= clSetKernelArg(kernel, 6, sizeof(unsigned int), &k);
    err |= clSetKernelArg(kernel, 7, sizeof(unsigned int), &bl_size);
